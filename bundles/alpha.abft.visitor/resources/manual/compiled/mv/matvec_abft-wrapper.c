...
//function prototypes
void matvec_abft(long, float**, float*, float*, float*);
void matvec_abft_verify(long, float**, float*, float*, float*);
...
int main(int argc, char** argv) {
	...
	{
		#if defined (RANDOM)
			#define S0(i) (x(i) = rand()) 
		#elif defined (CHECKING) || defined (VERIFY)
			#ifdef NO_PROMPT
				#define S0(i) scanf("%f", &x(i))
			#else
				#define S0(i) printf("x(%ld)=",(long) i); scanf("%f", &x(i))
			#endif
		#else
			#define S0(i) (x(i) = 1)   //Default value
		#endif
	...
	matvec_abft(N, A, x, b, &check_b_i_0);
	...
	//Verification Run
			...
			elapsed_time = (((double) time.tv_sec) + ((double) time.tv_usec)/1000000);
		#endif
    	matvec_abft_verify(N, A, x, b_verify, &check_b_i_0_verify);
    	#ifdef TIMING
    		gettimeofday(&time, NULL);
			elapsed_time = (((double) time.tv_sec) + ((double) time.tv_usec)/1000000) - elapsed_time;
		...
		
		{
			...
			#else
				#define S0(i) printf("b(%ld)=",(long) i);printf("%0.2f\n",var_b(i))
			#endif
			int c1;
			for(c1=0;c1 <= N-1;c1+=1)
			 {
		...
		//Compare outputs for verification
		{
			//Error Counter
			int _errors_ = 0;
			#define S0(i) if (fabsf(1.0f - var_b_verify(i)/var_b(i)) > EPSILON) _errors_++;
			int c1;
			for(c1=0;c1 <= N-1;c1+=1)
			 {
			 	S0((c1));
