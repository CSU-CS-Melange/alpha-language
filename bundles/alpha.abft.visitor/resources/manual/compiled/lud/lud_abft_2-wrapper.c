...
#define Inv_L_r_verify(i) Inv_L_r_verify[i]
#define Inv_U_c_verify(j) Inv_U_c_verify[j]
...
#define var_Inv_L_r(i) Inv_L_r(i)
#define var_Inv_L_r_verify(i) Inv_L_r_verify(i)
#define var_Inv_U_c(j) Inv_U_c(j)
#define var_Inv_U_c_verify(j) Inv_U_c_verify(j)
#define var_Inv_U_r(i) Inv_U_r(i)
#define var_Inv_U_r_verify(i) Inv_U_r_verify(i)

//function prototypes
void lud_abft_2(long, float**, float**, float**, float*, float*, float*, float*);
void lud_abft_2_verify(long, float**, float**, float**, float*, float*, float*, float*);
...
int main(int argc, char** argv) {
	...
	float* Inv_L_r = (float*)malloc(sizeof(float)*(N));
	mallocCheck(Inv_L_r, (N), float);
	float* Inv_U_c = (float*)malloc(sizeof(float)*(N));
	mallocCheck(Inv_U_c, (N), float);
	...
	mallocCheck(Inv_U_r, (N), float);
		...
		float* Inv_L_r_verify = (float*)malloc(sizeof(float)*(N));
		mallocCheck(Inv_L_r_verify, (N), float);
		float* Inv_U_c_verify = (float*)malloc(sizeof(float)*(N));
		mallocCheck(Inv_U_c_verify, (N), float);
	...
	lud_abft_2(N, A, L, U, Inv_L_c, Inv_L_r, Inv_U_c, Inv_U_r);
	...
	//Verification Run
			...
			elapsed_time = (((double) time.tv_sec) + ((double) time.tv_usec)/1000000);
		#endif
    	lud_abft_2_verify(N, A, L_verify, U_verify, Inv_L_c_verify, Inv_L_r_verify, Inv_U_c_verify, Inv_U_r_verify);
    	#ifdef TIMING
    		gettimeofday(&time, NULL);
			elapsed_time = (((double) time.tv_sec) + ((double) time.tv_usec)/1000000) - elapsed_time;
		...
		
		{
			...
			#else
				#define S0(i) printf("Inv_L_r(%ld)=",(long) i);printf("%0.2f\n",var_Inv_L_r(i))
			#endif
			int c1;
			for(c1=0;c1 <= N-1;c1+=1)
			 {
		...
		
		{
				...
				#define S0(j) printf("%0.2f\n",var_Inv_U_c(j))
			#else
				#define S0(j) printf("Inv_U_c(%ld)=",(long) j);printf("%0.2f\n",var_Inv_U_c(j))
			#endif
			int c1;
			for(c1=0;c1 <= N-1;c1+=1)
			 {
		...
		}
		{
			//Error Counter
			int _errors_ = 0;
			#define S0(i) if (fabsf(1.0f - var_Inv_L_r_verify(i)/var_Inv_L_r(i)) > EPSILON) _errors_++;
			int c1;
			for(c1=0;c1 <= N-1;c1+=1)
			 {
			 	S0((c1));
			...
			#undef S0
			if(_errors_ == 0){
				printf("TEST for Inv_L_r PASSED\n");
			}else{
				printf("TEST for Inv_L_r FAILED. #Errors: %d\n", _errors_);
			}
		}
		{
			//Error Counter
			int _errors_ = 0;
			#define S0(j) if (fabsf(1.0f - var_Inv_U_c_verify(j)/var_Inv_U_c(j)) > EPSILON) _errors_++;
			int c1;
			for(c1=0;c1 <= N-1;c1+=1)
			 {
			 	S0((c1));
			...
			#undef S0
			if(_errors_ == 0){
				printf("TEST for Inv_U_c PASSED\n");
			}else{
				printf("TEST for Inv_U_c FAILED. #Errors: %d\n", _errors_);
			}
		}
		{
			//Error Counter
