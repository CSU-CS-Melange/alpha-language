...
#define check_B_i_0(i) check_B_i_0[((-1 + N - i >= 0 && -1 + i >= 0) ? (i) : 0)]
#define check_B_i_1(i) check_B_i_1[((-1 + N - i >= 0 && -1 + i >= 0) ? (i) : 0)]
#define check_B_j_0(j) check_B_j_0[((-1 + N - j >= 0 && -1 + j >= 0) ? (j) : 0)]
#define check_B_j_1(j) check_B_j_1[((-1 + N - j >= 0 && -1 + j >= 0) ? (j) : 0)]
...
void scalmat_aabft(long _local_N, float* _local_k, float** _local_A, float** _local_B, float* _local_check_B_i_inv, float* _local_check_B_j_inv);
...
void scalmat_aabft(long _local_N, float* _local_k, float** _local_A, float** _local_B, float* _local_check_B_i_inv, float* _local_check_B_j_inv) {
	...
	check_B_i_0 = (float*)(malloc((sizeof(float)) * (((-1 + N >= 0) ? (N) : 0))));
	mallocCheck(check_B_i_0,"check_B_i_0");
	check_B_i_1 = (float*)(malloc((sizeof(float)) * (((-1 + N >= 0) ? (N) : 0))));
	mallocCheck(check_B_i_1,"check_B_i_1");
	check_B_j_0 = (float*)(malloc((sizeof(float)) * (((-1 + N >= 0) ? (N) : 0))));
	mallocCheck(check_B_j_0,"check_B_j_0");
	check_B_j_1 = (float*)(malloc((sizeof(float)) * (((-1 + N >= 0) ? (N) : 0))));
	mallocCheck(check_B_j_1,"check_B_j_1");
	...
	#undef S0
	#define S1(i) eval_check_B_i_inv(i)
	for (i = 0; i < N; i += 1) {
		S1(i);
	}
	#undef S1
	#define S2(j) eval_check_B_j_inv(j)
	for (j = 0; j < N; j += 1) {
		S2(j);
	}
	#undef S2
