...
float reduce_fsub_verify_x_1(long, int);
...
void fsub_verify(long N, float** _local_L, float* _local_b, float* _local_x){
	///Parameter checking
	if (!((N >= 1))) {
...
float reduce_fsub_verify_x_1(long N, int ip){
	float reduceVar = 0;
...
float eval_verify_x(long N, int i){
	if ( _flag_x(i) == 'N' ) {
		_flag_x(i) = 'I';
	//Body for x
		x(i) = (((i == 0))?(b(i))/(L(i,i)):(((b(i))-(reduce_fsub_verify_x_1(N,i)))/(L(i,i))));
		_flag_x(i) = 'F';
	} else if ( _flag_x(i) == 'I' ) {
