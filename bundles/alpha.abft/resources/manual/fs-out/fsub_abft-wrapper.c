...
#define Inv_b_c_verify(s) Inv_b_c_verify[s]
#define var_x(i) x(i)
#define var_x_verify(i) x_verify(i)
#define var_Inv_b_c(s) Inv_b_c(s)
#define var_Inv_b_c_verify(s) Inv_b_c_verify(s)
...
int main(int argc, char** argv) {
	...
	float* Inv_b_c = (float*)malloc(sizeof(float)*(1));
	mallocCheck(Inv_b_c, (1), float);
	#ifdef VERIFY
		float* x_verify = (float*)malloc(sizeof(float)*(N));
		mallocCheck(x_verify, (N), float);
		float* Inv_b_c_verify = (float*)malloc(sizeof(float)*(1));
		mallocCheck(Inv_b_c_verify, (1), float);
	#endif
	...
	//Verification Run
			...
			elapsed_time = (((double) time.tv_sec) + ((double) time.tv_usec)/1000000);
		#endif
    	fsub_abft_verify(N, L, b, x_verify, Inv_b_c_verify);
    	#ifdef TIMING
    		gettimeofday(&time, NULL);
			elapsed_time = (((double) time.tv_sec) + ((double) time.tv_usec)/1000000) - elapsed_time;
		...
		
		{
			...
			#else
				#define S0(s) printf("Inv_b_c(%ld)=",(long) s);printf("%0.2f\n",var_Inv_b_c(s))
			#endif
			S0((0));
			#undef S0
		}
		...
		}
		{
			//Error Counter
			int _errors_ = 0;
			#define S0(s) if (fabsf(1.0f - var_Inv_b_c_verify(s)/var_Inv_b_c(s)) > EPSILON) _errors_++;
			S0((0));
			#undef S0
			if(_errors_ == 0){
