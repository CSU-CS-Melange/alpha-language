...
//function prototypes
void matmult_abft(long, float**, float**, float**, float*);
void matmult_abft_verify(long, float**, float**, float**, float*);
...
int main(int argc, char** argv) {
	...
	//Verification Run
			...
			elapsed_time = (((double) time.tv_sec) + ((double) time.tv_usec)/1000000);
		#endif
    	matmult_abft_verify(N, A, B, C_verify, Inv_C_i_verify);
    	#ifdef TIMING
    		gettimeofday(&time, NULL);
			elapsed_time = (((double) time.tv_sec) + ((double) time.tv_usec)/1000000) - elapsed_time;
