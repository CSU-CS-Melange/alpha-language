...
class AlphaUtil {
	...
	static def Iterator<ReduceExpression> getContainedReductions(AlphaVisitable av) {
		return EcoreUtil.getAllContents(av, false)
			.filter[it instanceof ReduceExpression]
			.map[it as ReduceExpression]
	}
	
	/** Gets a unique name for a given reduction, e.g. for use in codegen */
	static def String getReductionName(ReduceExpression re) {
		val eq = re.getContainerEquation as StandardEquation
		val reductions = eq.getContainedReductions.toList
		
		return eq.variable.name + "_reduce" + reductions.indexOf(re)
	}
	
	/** Gets a reduction provided the unique name generated by the above method */
	static def ReduceExpression getReductionByName(SystemBody body, String name) {
		val variableName = name.substring(0, name.lastIndexOf("_reduce"))
		val reduceID = Integer.parseInt(
			name.substring(name.indexOf("_reduce") + "_reduce".length)
		)
		val eq = body.getStandardEquation(variableName)
		val reductions = eq.getContainedReductions.toList
