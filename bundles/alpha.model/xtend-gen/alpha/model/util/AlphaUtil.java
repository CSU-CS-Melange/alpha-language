...
public class AlphaUtil {
  ...
  /**
   * Gets all the reductions descended from an AlphaVisitable
   */
  public static Iterator<ReduceExpression> getContainedReductions(final AlphaVisitable av) {
    final Function1<Object, Boolean> _function = (Object it) -> {
      return Boolean.valueOf((it instanceof ReduceExpression));
    };
    final Function1<Object, ReduceExpression> _function_1 = (Object it) -> {
      return ((ReduceExpression) it);
    };
    return IteratorExtensions.<Object, ReduceExpression>map(IteratorExtensions.<Object>filter(EcoreUtil.<Object>getAllContents(av, false), _function), _function_1);
  }

  /**
   * Gets a unique name for a given reduction, e.g. for use in codegen
   */
  public static String getReductionName(final ReduceExpression re) {
    ...
    final List<ReduceExpression> reductions = IteratorExtensions.<ReduceExpression>toList(AlphaUtil.getContainedReductions(eq));
    String _name = eq.getVariable().getName();
    String _plus = (_name + "_reduce");
    int _indexOf = reductions.indexOf(re);
    return (_plus + Integer.valueOf(_indexOf));
  }

  /**
   * Gets a reduction provided the unique name generated by the above method
   */
  public static ReduceExpression getReductionByName(final SystemBody body, final String name) {
    final String variableName = name.substring(0, name.lastIndexOf("_reduce"));
    int _indexOf = name.indexOf("_reduce");
    int _length = "_reduce".length();
    int _plus = (_indexOf + _length);
    final int reduceID = Integer.parseInt(
      name.substring(_plus));
    final StandardEquation eq = body.getStandardEquation(variableName);
    final List<ReduceExpression> reductions = IteratorExtensions.<ReduceExpression>toList(AlphaUtil.getContainedReductions(eq));
    return reductions.get(reduceID);
  }
