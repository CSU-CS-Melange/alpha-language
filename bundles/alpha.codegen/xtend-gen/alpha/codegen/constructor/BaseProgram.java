...
public abstract class BaseProgram extends AbstractAlphaCompleteVisitor {
  ...
  public boolean defaultGlobalMacros() {
    boolean _xblockexpression = false;
    {
      final List<List<String>> macros = Collections.<List<String>>unmodifiableList(CollectionLiterals.<List<String>>newArrayList(Collections.<String>unmodifiableList(CollectionLiterals.<String>newArrayList("max(a,b)", "((a)>(b)?(a):(b))")), Collections.<String>unmodifiableList(CollectionLiterals.<String>newArrayList("min(a,b)", "((a)>(b)?(b):(a))")), Collections.<String>unmodifiableList(CollectionLiterals.<String>newArrayList("ceild(a,b)", "(int)ceil(((double)(a))/((double)(b)))")), Collections.<String>unmodifiableList(CollectionLiterals.<String>newArrayList("floord(a,b)", "(int)floor(((double)(a))/((double)(b)))")), Collections.<String>unmodifiableList(CollectionLiterals.<String>newArrayList("mod(a,b)", "((a)%(b))")), Collections.<String>unmodifiableList(CollectionLiterals.<String>newArrayList("mallocCheck(v,s)", "if ((v) == NULL) { printf(\"Failed to allocate memory for variable: %s\\n\", s); exit(-1); }"))));
